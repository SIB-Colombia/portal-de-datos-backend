...
"aggs" : {
   providers: {
    ...
    terms: {field: 'provider.id',size:100},
    
    aggs: {
      provider: {top_hits: {size: 1, _source: {include: ['provider']}}
    }
  }
    }}
  ...
  body: {
    size:1,
    "query": { "match": { "provider.id": providerId} },
"aggs" : {
  // filter : { match : { "provider.name" : "Asociación de Becarios del Casanare - ABC"   }},
...
//issues 6 
function CountsGeneral(req, res) {
  client.search({
    index: 'sibdataportal',
    type: 'occurrence',
  body: {
    "aggs":{
      "quantity_stats_Provider":{
        
                "stats":{
                  "field":"provider.id"
                }
              },
      "quantity_stats_Resource":{
                
                        "stats":{
                          "field":"resource.id"
                        }
                      },
                      "quantity_stats_species":{
                        
                                "stats":{
                                  "field":"taxonomy"
                                }
                              },
                              "quantity_stats_location":{
...
//issues 7 - 8
function speciesGroup(req, res) {
  client.search({
    index: 'sibdataportal',
    type: 'occurrence',
  body: {
    "from" : 0, "size" : 1500,
    "_source": ["provider","taxonomy"]
    }
  }, (err, response) => {
    if (err) {
      res.status(400).json({
...
//issues 10 
function resourcesGroup(req, res) {
  client.search({
    index: 'sibdataportal',
    type: 'occurrence',
  body: {
    "aggs" : {
      resources: {
       ...
       terms: {field: 'resource.id',size:999},
       
       aggs: {
        resource: {top_hits: {size: 1, _source: {include: ['resource']}}
       }
     }
       }}
    }
...
//issue 12
function Occurrencesearch(req, res) {
...
//issue 9
function occurrencesGroupCount(req, res) {
...
"aggs" : {
   providers: {
    ...
    terms: {field: 'provider.id',size:200},
...
terms: {field: 'resource.id',size:1000},
